...
const KEYWORD_SCOPES = [
  ...
  ['junction rule', "Kirchhoff's Rules"],
  ['loop rule', "Kirchhoff's Rules"],
  ['emf', 'EMF and Internal Resistance'],
  ['internal resistance', 'EMF and Internal Resistance'],
...
export function detectScopes({ files = [], examContext = '' } = {}) {
  ...
  KEYWORD_SCOPES.forEach(([keyword, label]) => {
...
function buildModelInput(files, examContext, selectedScopes, detectedScopes) {
  const content = [];
  ...
  content.push({
    type: 'input_text',
    text: [
      'Build a focused physics study pack from the uploaded class material.',
...
export async function generateStudyPack({ files = [], examContext = '', selectedScopes = [], detectedScopes = [] }) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      model: DEFAULT_MODEL,
      instructions: [
        'You generate focused study packs for a gamified physics study app.',
        ...
        'Formula cards should explain variables and physical meaning.',
      ...
      input: buildModelInput(files, examContext, selectedScopes, detectedScopes),
      tools: [
        {
